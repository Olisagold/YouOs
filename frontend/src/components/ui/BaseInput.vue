...
<template>
  <div class="space-y-2">
    <label v-if="label" class="block text-sm font-medium text-[var(--text)]">
      {{ label }}
    ...
    <p v-if="error" class="text-xs text-[rgb(188,125,125)]">
      {{ error }}
    </p>
    <p v-else-if="hint" class="text-xs text-[var(--muted)]">
      {{ hint }}
    </p>
  </div>
</template>
